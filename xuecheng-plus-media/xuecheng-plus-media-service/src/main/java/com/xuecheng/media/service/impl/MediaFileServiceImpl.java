...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public RestResponse<Boolean> checkChunk(String fileMd5, int chunkIndex) {
        //1.分块先查数据库得到文件bucket，objectName
        //分块路径，为md5前两位目录加md5的值
        MediaFiles mediaFiles = mediaFilesMapper.selectById(fileMd5);
        if (mediaFiles != null) {
            //2.数据库有再查minio
            //得到分块文件目录
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localPath) {
        ...
        try {
            //将文件存储至minIO
            boolean b = addMediaFilesToMinIO(localPath, mimeType, bucket_videos, chunkFilePath);
            if (!b) {
